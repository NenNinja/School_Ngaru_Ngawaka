...
class BlogDB:
    """Class to manage blog-related database operations."""
    ...
    def get_post(self, title):
        ...
        self.cursor.execute(
            "SELECT u.username, p.title, p.content, p.timestamp "
    ...
    def get_posts(self, username):
        ...
        if not author_id:
            raise ValueError("User not found")
        self.cursor.execute(
            "SELECT title, content, timestamp FROM posts "
            "WHERE author_id=? ORDER BY timestamp DESC",
            (author_id[0],),
        )
    ...
    def delete_post(self, username, title):
        ...
        if not author_id:
            raise ValueError("User not found")
        self.cursor.execute(
            "DELETE FROM posts WHERE author_id=? AND title=?",
            (author_id[0], title),
        )
        self.conn.commit()
